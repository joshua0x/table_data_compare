...
func ParseSql(sql string, options ...Option) (ModelCodes, error) {
	...
	for _, stmt := range stmts {
		if ct, ok := stmt.(*ast.CreateTableStmt); ok {
			s, ipt,tname, err := makeCode(ct, opt)
			if err != nil {
				return ModelCodes{}, err
			}
			tcamelName = tname
	...
	return ModelCodes{
		...
		StructCode: tableStr,
		PkColName: pkName,
		TableCamelName: tcamelName,
	}, nil
}

func getPkName(node *ast.CreateTableStmt) string{
	/*
	...
	*/
	for _,col := range node.Constraints {
		if col.Tp == ast.ConstraintPrimaryKey {
			return col.Keys[0].Column.String()
		}
	}
	return ""
}
...
func makeCode(stmt *ast.CreateTableStmt, opt options) (string, []string,string, error) {
	...
	if err != nil {
		return string(code), importPath,data.TableName, errors.WithMessage(err, "format golang code error")
	}
